...
warm-burnout/
  ...
  Cargo.toml                  # Rust workspace: test harness + JetBrains codegen tool
  Cargo.lock
  rustfmt.toml                # 2-space indent, 120 max width
  justfile                    # jetbrains-build / jetbrains-sync / jetbrains-clean recipes
  src/lib.rs
  tools/
    warm-burnout-codegen/     # Codegen binary for the JetBrains templates
  scripts/
    jetbrains-sync.sh         # Diff JetBrains/rider-theme-pack against the pinned revision
  tests/
    ...
    contrast.rs               # WCAG contrast ratio checks for the canonical palette
    ...
    nvim_load_test.lua        # Lua helper loaded by nvim.rs
    vim_load_test.vim         # Vimscript helper loaded by vim.rs
  .github/
    FUNDING.yml               # GitHub Sponsors config
    workflows/
      validate.yml            # CI: fmt, clippy, tests, rumdl on push/PR
      ...
      deploy-site.yml         # GitHub Pages deploy for site/
  themes/
  ...
  vscode/                     # VS Code extension (primary, palette source of truth)
    ...
    package.json
    .vscodeignore
    warm-burnout-logo-vscode.png  # Marketplace icon, generated by screenshots/generate-logo.mjs
    themes/
  ...
  helix/                      # Helix editor theme
    README.md                 # Helix install instructions
    AGENTS.md                 # Helix-specific agent rules
  ...
  screenshots/                # Theme preview screenshots
    ...
    generate.mjs              # Playwright script to render the mockups -> PNG
    generate-logo.mjs         # Playwright script for the OG image + VS Code marketplace icon
    ...
    og-image.html             # Social/OG image mockup -> site/og-image.png
    logo-vscode.html          # Marketplace icon mockup -> vscode/warm-burnout-logo-vscode.png
    *.png                     # Generated 2400x1600 @2x retina PNGs
  images/
    wb-logo.svg               # Brand logo (site favicon + header)
  site/                       # GitHub Pages website (warmburnout.com), deployed by deploy-site.yml
    index.html                # Single-page site
    css/input.css             # Tailwind v4 source (css/output.css is generated, gitignored)
    js/main.js                # Tagline rotation, theme toggle
    logos/                    # Logos for the "used by" section
    og-image.png              # Generated by screenshots/generate-logo.mjs
    package.json              # npm run build -> Tailwind CLI
    CNAME                     # Custom domain (warmburnout.com)
    robots.txt
    sitemap.xml
```
...
7. Update the platforms table in the root `README.md` and the "Also Available For" lists in `vscode/README.md` and `zed/README.md`.
8. Update the platforms grid and the JSON-LD `softwareRequirements` list in `site/index.html` (the GitHub Pages website).
...
- `jetbrains/README.md` (absolute `raw.githubusercontent.com` URL)
- `site/index.html` (GitHub Pages website, copied into `_site/screenshots/` by the deploy workflow)

When adding a new screenshot, update every README that embeds screenshots and the `PAGES` array in `generate.mjs`.

The OG image and the VS Code marketplace icon are separate: `node generate-logo.mjs` renders `og-image.html` into `site/og-image.png` and `logo-vscode.html` into `vscode/warm-burnout-logo-vscode.png`.
